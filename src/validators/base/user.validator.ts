...
export default class UserBaseValidator extends BaseValidator {
  ...
  async runValidations() {
    ...
    if (!this.modelInstance.id || this.attrs.password) {
      if (typeof password !== 'string') { this.errors.add('password', 'presence'); }
      if (typeof password === 'string' && password.length < this.minPasswordLength) {
    ...
    if (typeof email === 'string' && !isEmail(email)) { this.errors.add('email', 'format'); }
    const isEmailUniq = await (this.isEmailUniq(email));

    if (!isEmailUniq) { this.errors.add('email', 'uniq'); }
  }
